...
const FINGERPRINT_META = {
  'Ghost Load': {
    type: 'Ghost Load', bc: 'bg-ghost', icon: '🌙', chart: 'ghost',
  ...
  'Phase Bypass': {
    type: 'Phase Bypass', bc: 'bg-phase', icon: '⚡', chart: 'phase',
  ...
  'Billing Freeze': {
    type: 'Billing Freeze', bc: 'bg-freeze', icon: '🔒', chart: 'freeze',
  ...
  'Meter Tamper': {
    type: 'Meter Tamper', bc: 'bg-tamper', icon: '🔧', chart: 'tamper',
  ...
  'Slow Bleed': {
    type: 'Slow Bleed', bc: 'bg-bleed', icon: '📉', chart: 'bleed',
...
async function fetchMeters() {
  try {
    const res = await fetch('https://meter-mind.onrender.com/api/fingerprints/meters');
...
async function pick(idx) {
  ...
  try {
    const res = await fetch(`https://meter-mind.onrender.com/api/fingerprints/meters/${m.id}`);
    const data = await res.json();
    drawRealChart(
